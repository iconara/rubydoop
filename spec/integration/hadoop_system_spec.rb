...
describe 'Packaging and running a project' do
  ...
  context 'Running the project' do
    before :all do
      isolated_run(TEST_PROJECT_DIR, "HADOOP_OPTS=\"$HADOOP_OPTS -Dpuck.entrypoint=rubydoop-runner\" #{HADOOP_HOME}/bin/hadoop jar build/test_project.jar -conf conf/hadoop-local.xml test_project data/input data/output 2>&1 | tee data/log")
    end
